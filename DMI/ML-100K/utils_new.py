...
def split_lp_train_test_matrix():

    cnt_U = Counter()
    cnt_V = Counter()

    print('> reading files')
    with open('dblp/edges.dat') as f:
        for line in f:
    ...
    save_pickle(u_mapping, 'dblp/u_mapping.pickle')
    save_pickle(v_mapping, 'dblp/v_mapping.pickle')
    ...
    with open('dblp/dblp_edges.dat') as f:
        for line in f:
            u, v, weight = line.split()
    ...
    # save files
    print('> saving file')
    save_pickle(csr_train, 'dblp/train_lp.csr.pickle')
    print('save to', 'dblp/train_lp.csr.pickle')
    save_pickle(csr_test, 'dblp/test_lp.csr.pickle')
    print('save to', 'dblp/test_lp.csr.pickle')
...
def split_RS_train_test_matrix():
    ...
    # train = edges[trnIndices]

    # valIndices = indices[trn:val]
    # val_edges = edges[valIndices]

    # tstIndices = indices[val:]
    # test = edges[tstIndices]

    train_edges = pd.read_csv('dataset/train.txt', header=None, sep='\t')
    test_edges = pd.read_csv('dataset/test.txt', header=None, sep='\t')

    train_src = np.array(train_edges.iloc[:, 0])
    train_dst = np.array(train_edges.iloc[:, 1])
    test_src = np.array(test_edges.iloc[:, 0])
    test_dst = np.array(test_edges.iloc[:, 1])

    train = np.stack((train_src, train_dst), axis=-1)
    test = np.stack((test_src, test_dst), axis=-1)

    edges = np.vstack((train, test))

    num_U, num_V = 943, 1682
    ...
    csr_all = csr_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])), shape=(num_U, num_V))
    csr_train = csr_matrix((np.ones(train.shape[0]), (train[:, 0], train[:, 1])), shape=(num_U, num_V))
    csr_test = csr_matrix((np.ones(test.shape[0]), (test[:, 0], test[:, 1])), shape=(num_U, num_V))


    print('all graph:', csr_all.shape, csr_all.nnz)
    print('train graph:', csr_train.shape, csr_train.nnz)
    print('test graph:', csr_test.shape, csr_test.nnz)

    save_pickle(csr_all, 'dataset/all_RS.csr.pickle')
    print('save to', 'dataset/all_RS.csr.pickle')
    save_pickle(csr_train, 'dataset/train_RS.csr.pickle')
    print('save to', 'dataset/train_RS.csr.pickle')
    save_pickle(csr_test, 'dataset/test_RS.csr.pickle')
    print('save to', 'dataset/test_RS.csr.pickle')
...
################################### train data ############################
##### link prediction
def get_LP_train_test_edges():      
    # train_ratio = 1.5
    
    with open('dblp/train_lp.csr.pickle', 'rb') as f:
        csr_train = pickle.load(f)
        print('train:', csr_train.shape, csr_train.nnz)
    with open('dblp/test_lp.csr.pickle', 'rb') as f:
    ...
    # n_train_pos = int(train_ratio * n_test_pos)
    n_train_pos = csr_train.nnz
    ...
    return num_U, num_V, train_data_loader, test_data_loader, csr_train, csr_test
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.num_negs > 0:
            ...
            neg_idx = np.random.randint(self.num_V, size=20*self.num_negs)
...
def get_RS_train_test_edges(n_negs = 10):
    ...
    with open('dataset/all_RS.csr.pickle', 'rb') as f:
        csr_all = pickle.load(f)
        print('all:', csr_all.shape, csr_all.nnz)
    with open('dataset/train_RS.csr.pickle', 'rb') as f:
        csr_train = pickle.load(f)
        print('train:', csr_train.shape, csr_train.nnz)
    with open('dataset/test_RS.csr.pickle', 'rb') as f:
        csr_test = pickle.load(f)
        print('test:', csr_test.shape, csr_test.nnz)
...
def ranking_edges(TEST_LHS, TEST_RHS, eval_model, epoch_id, SET_ALPHA):
    eval_model.eval()
...
def batch_evaluation(trained_model, csr_test, csr_train, epoch, test_batch, topk, max_K, set_alpha, device='cuda:0'):
    ...
    with tqdm(total=csr_test.shape[0], desc=f'eval epoch {epoch}') as pbar:
        for i, batch in enumerate(test_minibatch(csr_test, csr_train, test_batch)):
            ...
            with torch.no_grad():
                ...
                ##################################### 修改
                rating = ranking_edges(test_lhs, V_EMDS, trained_model, epoch, set_alpha)
