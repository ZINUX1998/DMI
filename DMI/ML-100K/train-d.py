...
train_data_loader, train_csr, test_csr = get_RS_train_test_edges(2)
...
class get_node_emb(nn.Module):
    def __init__(self, NUM_U, NUM_V, U_CLUSTER_ARC, V_CLUSTER_ARC, IN_DIM):
...
class update_nodes_embedding(nn.Module):
    def __init__(self, NODES_U_NUMBER, NODES_V_NUMBER, U_CLUSTERS_DIM, V_CLUSTERS_DIM, SET_D):
        super(update_nodes_embedding, self).__init__()
        self.prediction_module = get_node_emb(NODES_U_NUMBER,
                                              NODES_V_NUMBER,
                                              U_CLUSTERS_DIM, 
                                              V_CLUSTERS_DIM, SET_D).cuda()
    ...
    def forward(self, DATA_TRAIN, TRAIN_CSR, TEST_CSR, TEST_BETCH, 
                TOP_K, MAX_K, SET_ALPHA, SET_BETA, print_max_K = 0):
...
cluster_number = 32
max_K = max(topk)
U_hidden_dim, U_output_dim, V_hidden_dim, V_output_dim = 24, 48, 24, 48
...
if __name__ == '__main__':
    for setting_d in [16, 32, 48, 64, 80, 96, 112, 128]:
        print('################################ dim: ', setting_d)
        UPDATE_NODES_MODULE = update_nodes_embedding(NUMBER_U, NUMBER_V, U_network_arc, V_network_arc, setting_d)
        best_result = UPDATE_NODES_MODULE(train_data_loader, train_csr, test_csr, test_batch, topk, max_K, setting_alpha, setting_beta)
