...
def split_RS_train_test_matrix():
    ...
    save_pickle(csr_all, 'dblp/all_RS.csr.pickle')
    print('save to', 'dblp/all_RS.csr.pickle')
    save_pickle(csr_train, 'dblp/train_RS.csr.pickle')
    print('save to', 'dblp/train_RS.csr.pickle')
    save_pickle(csr_test, 'dblp/test_RS.csr.pickle')
    print('save to', 'dblp/test_RS.csr.pickle')
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.num_negs > 0:
            ...
            neg_idx = np.random.randint(self.num_V, size=6*self.num_negs)
...
def get_RS_train_test_edges(n_negs = 10):
    ...
    with open('dblp/train.csr.pickle', 'rb') as f:
        csr_train = pickle.load(f)
        print('train:', csr_train.shape, csr_train.nnz)
    with open('dblp/test.csr.pickle', 'rb') as f:
        csr_test = pickle.load(f)
        print('test:', csr_test.shape, csr_test.nnz)
...
def batch_evaluation(trained_model, csr_test, csr_train, epoch, test_batch, topk, max_K, SETING_ALPHA, device='cuda:0'):
    ...
    with tqdm(total=csr_test.shape[0], desc=f'eval epoch {epoch}') as pbar:
        for i, batch in enumerate(test_minibatch(csr_test, csr_train, test_batch)):
            ...
            with torch.no_grad():
                ...
                ##################################### 修改
                rating = ranking_edges(test_lhs, V_EMDS, trained_model, epoch, SETING_ALPHA)
