...
def get_LP_train_test_data(n_negs = 10):
    
    with open('sparse_yelp/train.csr.pickle', 'rb') as f:
        csr_train = pickle.load(f)
        print('train:', csr_train.shape, csr_train.nnz)
    with open('sparse_yelp/test.csr.pickle', 'rb') as f:
    ...
    edges = lp_pos_test.tolist() + lp_neg_test.tolist() + train.tolist()
    ...
    return num_U, num_V, train_dataloader, test_data_loader
...
def predict_edges(TEST_LHS, TEST_RHS, eval_model):
    eval_model.eval()
    ...
    with torch.no_grad():
        ...
        LINK_PROB = 0.7*PROB_CLUSTER + 0.3*PROB_U_V
