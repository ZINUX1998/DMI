...
train_data_loader, train_csr, test_csr = get_RS_train_test_edges(10)
...
#                     print('iter loss: ', loss.item())
            ...
            if epoch == 0:
                best_cluster = self.prediction_module.STRUCTURE_CLUSTERS.weight.detach()
        ...
        return best_metrics, best_cluster.cpu()
...
if __name__ == '__main__':
    ...
    for cluster_number in [16]:
        print('##########################cluster number: ', cluster_number)
        
        U_hidden_dim, U_output_dim, V_hidden_dim, V_output_dim = 32, 32, 32, 32
        ...
        best_result, final_clusters = UPDATE_NODES_MODULE(train_data_loader, train_csr, test_csr, test_batch, topk, max_K, setting_alpha)
        
        print('clusters embeddings: ', final_clusters)
        df = pd.DataFrame(final_clusters.numpy())
        df.to_csv('clusters_output'+str(cluster_number)+'.txt', header=False, index=False, sep=' ', mode='w')
